  ...
  head: () => ({
    meta: [
      { title: "Blog — Blark-walter Designs" },
      {
        name: "description",
        content:
          "Insights, ideas and design stories on UI/UX, branding and product design by Clyde Walter.",
      },
      { property: "og:title", content: "Blog — Blark-walter Designs" },
      {
        property: "og:description",
        content: "Thoughts on UI/UX design, branding, product strategy and the creative process.",
      },
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_auto_1fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-md text-muted-foreground">
              Thoughts on UI/UX design, branding, product strategy, and the creative process. Real
              stories, practical tips, and insights to help you build better digital experiences.
            </p>
            <div className="mt-6 flex overflow-hidden rounded-full border border-border bg-card shadow-sm">
              ...
              />
              <button className="grid w-14 place-items-center bg-primary text-primary-foreground">
              ...
              <ul className="space-y-3">
                {posts.slice(0, 3).map((p) => (
                  <li key={p.slug} className="flex gap-3">
                    <div
                      className={`h-14 w-14 shrink-0 rounded-lg bg-gradient-to-br ${accentFor(p.category)}`}
                    />
                    <div>
                      <Link
                        to="/blog/$slug"
        ...
        <div className="ink-panel flex flex-col gap-6 p-8 text-ink-foreground md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-white/60">
              Get the latest articles, design tips, and resources straight to your inbox.
            </p>
          </div>
          <form className="flex w-full max-w-md gap-2" onSubmit={(e) => e.preventDefault()}>
            <input
              placeholder="Enter your email"
              className="w-full rounded-full bg-white/10 px-5 py-3 text-sm text-white placeholder:text-white/50 outline-none"
            />
