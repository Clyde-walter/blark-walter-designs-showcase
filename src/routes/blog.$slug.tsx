        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_320px]">
                ...
                <div key={sec.heading}>
                  <h2 className="mt-8 text-2xl font-bold">
                    ...
                    <span className="text-primary">
                      {sec.heading.split(" ").slice(-2).join(" ")}
                    </span>
                  </h2>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4 rounded-2xl border border-border bg-card p-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  UI/UX Designer &amp; Brand Designer passionate about creating user-centered
                  digital experiences that solve real problems and drive results.
                </p>
              </div>
            </div>
          </article>
          <aside className="space-y-6">
            <div className="rounded-2xl border border-border bg-card p-5 text-center">
              <div
                className={`mx-auto h-24 w-24 rounded-full bg-gradient-to-br ${accentFor(post.category)}`}
              />
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Helping startups and brands turn ideas into intuitive digital experiences.
              </p>
              <Link
                to="/about"
                className="mt-4 inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-xs font-semibold hover:border-primary hover:text-primary"
              >
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              <div className="mb-4 flex items-center gap-2 font-semibold">
                <span className="h-2 w-2 rounded-full bg-primary" /> Table of Contents
              </div>
            ...
            <div className="ink-panel p-5 text-ink-foreground">
              ...
              <div className="text-lg font-bold">
                Get the latest <span className="text-primary">design</span> insights
              </div>
              <p className="mt-2 text-xs text-white/60">
                Tips, resources, and stories straight to your inbox.
              </p>
              <form className="mt-4 space-y-2" onSubmit={(e) => e.preventDefault()}>
                <input
                  placeholder="Enter your email"
                  className="w-full rounded-lg bg-white/10 px-4 py-2.5 text-sm text-white placeholder:text-white/50 outline-none"
                />
        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-3">
          {related.map((p) => (
            <Link
              ...
              to="/blog/$slug"
              params={{ slug: p.slug }}
              className="block overflow-hidden rounded-2xl border border-border bg-card transition hover:-translate-y-1 hover:shadow-lg"
            >
